...
cat > /opt/data/webhook_subscriptions.json <<EOF
{
  "modern-collector-alerts": {
    ...
    "prompt": "Incidencia desde Modern Collector: id={id}; estado={status}; severidad={severity}; host={host}; tipo={kind}; servicio={service}; mensaje={message}. Aplica remediation-loop: consulta primero maintenance para el host y, si está en mantenimiento, informa sin corregir. Después reúne evidencia de solo lectura mediante Nightingale/OpenSRE. No abras tickets, no ejecutes SSH ni apliques cambios o correcciones sin confirmación explícita. Envía un resumen breve con evidencia y próximo paso.",
